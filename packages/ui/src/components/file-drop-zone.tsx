...
export interface DropzoneProps extends Omit<_DropzoneProps, "children"> {
  ...
  initialFiles?: File[]; // Add support for initial files
  onFileDelete?: () => void; // Callback when a file is deleted
}
...
const Dropzone = ({
  ...
  const dropzone = useDropzone({
    ...
    onDrop(acceptedFiles, fileRejections, event) {
      // Always update the internal state for UI display
      if (props.multiple === false) {
        // If multiple is false, only keep the latest file
        setFilesUploaded(acceptedFiles.slice(0, 1));
      } else {
